...
- [Nokia 5G Camera Integration](#nokia-5g-camera-integration)
...
1. **Dual-model pipeline** — Separates person detection (YOLOv8n) from vest classification (YOLOv11n) for higher accuracy than single-model approaches

2. **Threaded RTSP capture** — Decouples network I/O from inference to prevent frame blocking

3. **Frame dropping strategy** — Always processes the most recent frame, prioritizing latency over completeness for real-time safety

4. **Smooth bounding boxes** — Exponential moving average (α=0.85) prevents visual flickering common in frame-by-frame detection

5. **Violation transition logging** — Only logs when state changes from compliant → violated (avoids log spam)

6. **FP16 inference** — Halves GPU memory usage and nearly doubles throughput with negligible accuracy loss

7. **Dual output channels** — MQTT for IoT integration + HTTP for local dashboard, ensuring data reaches both cloud and local monitoring
